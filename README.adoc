...
[Crossplane in the CNCF](https://www.cncf.io/projects/crossplane/)
...
helm repo update
helm upgrade --install crossplane --create-namespace -n crossplane-system --set metrics.enabled=true crossplane-stable/crossplane --wait
kubectl api-resources --api-group=apiextensions.crossplane.io
----
...
* Crossplane provides an declarative API on top of the "cloud" resources
* Abstracting Compositions of resources behind a Crossplane resource
* Declarative
* So we don't write code
...
* Our Glue to Cloud/API specific CRDs
...
* Crossplane offers CRDs on top of Provieders CRDs
...
* Infra masquerades them via CompositeResourceDefinition
...
Before you get started you need an Provider for "Cloud" Ressources you want to access.

Providers are bundled as OCI-Images. There is also a list of Providers https://marketplace.upbound.io/
...
Erkan/Robert: Explain the "Offering Concept" (of Compositions) in Crossplane.
...
include::provider-kubernetes/00-Setup/README.adoc[]

// Managed Resources for K8s

include::provider-kubernetes/01-ManagedResources/README.adoc[]
